...
namespace LogWriter
{
	public partial class Form1 : Form
	{
		...
		private void button1_Click(object sender, EventArgs e)
		{
			...
			if (TryGetCoeffsFromForm(out double coeffAdmin, out double coefCargo, out double coeffWorker))
            {
				UpdateTable(coeffAdmin, coefCargo, coeffWorker, localList);
        ...
        {
            MessageBox.Show("Кнопка отключена");
            return;
            return;
            try
            {
            ...
            DateTime fromDate = makeFromDate(dateTimePicker1.Value);
            DateTime toDate = makeToDate(dateTimePicker2.Value);
            // UpdateTable(coeffAdmin, coeffCargo, coeffWorker, fromDate, toDate);
        }

        private bool TryGetCoeffsFromForm(out double coeffAdminOut, out double coeffCargoOut, out double coeffWorkerOut)
        {
			coeffAdminOut = coeffCargoOut = coeffWorkerOut = 0;
            ...
            else
            {
				...
				coeffAdminOut = coeffAdmin / 100;
				coeffCargoOut = coeffCargo / 100;
				coeffWorkerOut = coeffWorker / 100;
				return true;
            }
        }

        private void UpdateTable(double coeffAdmin, double coeffCargo, double coeffWorker, IEnumerable<LogPosition> logPositions)
		{
			...
			var groupsByAdminName = from p in logPositions
										// where (p.Time > fromDate && p.Time < toDate)
			...
			//Получуние итого для администарторов
			var groupsByCargoName = from p in logPositions
										// where (p.Time > fromDate && p.Time < toDate)
			...
			groupsByCargoName = from p in logPositions
									// where (p.Time > fromDate && p.Time < toDate)
								group p by p.CharacterName;
			foreach (var g in groupsByCargoName)
			{
			...
			//Получуние итого
			var groupsByOreType1 = from p in logPositions
									   // where (p.Time > fromDate && p.Time < toDate)
